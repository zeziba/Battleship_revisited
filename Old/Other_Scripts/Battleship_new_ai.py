...
class AI(Player):
    def __init__(self, difficulty, type_player):
        ...
        self.hit_map = []
        self.delta_move = [(1, 0), (-1, 0), (0, 1), (0, -1)]
    ...
    def med_difficulty(self):
        ...
        # Hunt(w/ parity) Mode
            ...
            pos_e = [
                (pos[0], pos[1]) for pos in self.even_moves if pos in self.moves_left
            ]
            if pos_e:
    ...
    def check_delta_moves(self, coord, magnitude):
        ...
        to_move = [
            ...
            for possible_move in [
                ...
                for move in [
                    ...
                    for c_x, c_y in self.delta_move
                ]
                if coord in self.moves_left
            ]
        ...
        #     for mag in range(magnitude):
        #         if (coord[0] + (c_x * mag), coord[1] + (c_y * mag)) in self._moves_left:
        #             all_moves += (coord[0] + (c_x * mag), coord[1] + (c_y * mag))
        # if magnitude == moves_possible:
    ...
    def valid_attack_move(self, coord):
        test = [
            item
            for sublist in [
                self.check_delta_moves(coord, ship_length)
                for ship_length in [
                    self.fleet[ship].get_length()
        ...
        # all_moves = []
        # fleet = [self.fleet[ship].get_length() for ship in range(len(self.fleet)) if not self.fleet[ship]()]
        # for number in fleet:
        #     all_moves += self.check_delta_moves(coord, number)
    ...
    def best_move(self):
        ...
        best_moves = [
            key for key in self.point_map.keys() if self.point_map[key] == max_value
        ]
    ...
    def check_adj_tiles(self, map_to_check):
        return [
            ...
            for d_x, d_y in self.delta_move
            if (x1 + d_x, y1 + d_y) in self.moves_left
        ]

    def adjust_for_hits(self):
        # Can create a for loop to run this over and over again
        moves = self.check_adj_tiles(self.hit_map)
        adj_move = self.check_adj_tiles(moves)
        more_adj_moves = self.check_adj_tiles(adj_move)
        even_more_adj_move = self.check_adj_tiles(more_adj_moves)
        most_adj_moves = self.check_adj_tiles(even_more_adj_move)
        for move in most_adj_moves:
            self.point_map[move] += 2
        for move in even_more_adj_move:
            self.point_map[move] += 2
        for move in more_adj_moves:
            self.point_map[move] += 2
